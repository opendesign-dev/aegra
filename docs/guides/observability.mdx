...
- `assistant_id` — present when the run executes an assistant; omitted for stateless runs that bind no assistant. Filter on it to separate assistants that share one graph but differ in config (cost, latency, and quality per assistant rather than per graph)
