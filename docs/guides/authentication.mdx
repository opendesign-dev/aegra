...
mean. One string is interpreted by the server itself:

| Permission | Effect |
|------------|--------|
| `runs:search:all` | Lets `POST /runs/search` and `POST /runs/count` span the whole deployment instead of the caller's own runs. Without it those endpoints are pinned to the caller at the SQL layer; there is no request field that can widen the scope. |

Grant it only to operator or admin identities: it lifts per-user isolation on run
metadata, which includes run inputs and outputs.
