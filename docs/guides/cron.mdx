...
- **A pending approval holds the schedule.** When a thread-bound cron's run pauses on `interrupt()` (or a static breakpoint), the thread goes `interrupted` and the next firings are skipped rather than queued — firing anyway would append input and advance the checkpoint, discarding the context the approver is looking at. After `CRON_APPROVAL_TIMEOUT_SECONDS` (default 24h, `0` waits forever) those runs are failed with an explicit reason and the schedule is released, so one unattended approval cannot stop the cron permanently. The checkpoint still holds the pending interrupt, so the next firing re-runs the paused node and asks again rather than skipping the approval — the schedule keeps ticking, but the work only proceeds once someone decides. A cancelled run also settles as `interrupted`; cancels are excluded from this wait (they carry `cancel_requested`), so stopping a run never shortens or trips another run's approval timeout. Resume with `command={"resume": [{"type": "accept" | "edit" | "response" | "ignore", "args": ...}]}` — see [Human-in-the-loop](/guides/human-in-the-loop).
